...
export function updateVessels(
  ...
  labelMode: "on" | "off" | "simplified" = "on",
) {
  ...
  queue.sort(
    (a, b) =>
      a.vessel.executions[0].startTime - b.vessel.executions[0].startTime,
  );
  ...
  vesselPositions.forEach((_state, vesselId) => {
    ...
    if (time > lastExecEnd) {
      if (vessel.id == 360) console.log("LAST EX:", vessel.id);
    ...
    labelMode !== "off"
      ? Array.from(vesselPositions.entries()).map(([id, pos]) => {
          ...
          return {
            ...
            geometry: { type: "Point", coordinates: [pos.lng, pos.lat] },
            properties: {
              title: labelMode === "simplified" ? `V${id}` : `Vessel ${id}`,
              subtitle:
                labelMode === "simplified" ? "" : (detail?.vessel_berth ?? ""),
            },
          };
        })
      : [];
